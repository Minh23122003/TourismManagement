...
const ChangeNews = ({route}) => {
    ...
    const [cateId, setCateId] = React.useState(news.news_category_id)
    const [nameImage, setNameImage] = React.useState(news.news_image[0].name)
    ...
    const Change = async () => {
        if(news.title===''){
            ...
            try {
                ...
                let res1 = await APIs.put(endpoints['changNews'], {
